...
const About = () => {
    return (
        <>
            <div className="about bg-[#D7D9DD] px-5 text-justify lg:px-40 py-10 lg:py-16">
                ...
                <div className="buttons flex flex-wrap mt-2 lg:mt-6">
                    {
                        ['Outcomes', 'Objectives', 'Department History'].map((item, index) => (
                            <button key={index} className='bg-[#F02D65] font-jost hover:shadow-2xl shadow-black transition-all duration-300 hover:scale-105 text-white font-bold py-2 px-4 rounded-lg mt-2 mr-2 lg:mt-0 lg:my-5 '>{item}</button>
                        ))
                    }
                </div>
            </div>
