        ...
        <>
            <motion.div
                ...
                <div className="buttons flex flex-wrap mt-2 lg:mt-6">
                    {
                        [{ title: 'Outcomes', link: '/features/outcomes' },
                        { title: 'Objectives', link: '/features/objectives' },
                        { title: 'Department History', link: '/features/departmentHistory' }]
                            .map((item, index) => (
                                <Link href={item.link} key={index}>
                                    <button className='bg-pink-500 font-jost hover:shadow-2xl shadow-black transition-all duration-300 hover:scale-105 text-white font-bold py-2 px-4 rounded-lg mt-2 mr-2 lg:mt-0 lg:my-5'>
                                        {item.title}
                                    </button>
                                </Link>
                            ))
