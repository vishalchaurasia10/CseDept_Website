...
const SignUp = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await account.create(ID.unique(), credentials.email, credentials.password, credentials.name, credentials.redirectURL,);
    ...
    const checkValidity = (e) => {
        ...
        } else if (credentials.secretKey !== process.env.NEXT_PUBLIC_SECRET_KEY) {
                ...
                <div className="content w-full md:w-[27%] border bg-pureWhite border-gray-300 p-6 rounded-2xl shadow-2xl shadow-black">
                    ...
                    <form className='form flex w-full flex-col'>
                        <div className="name flex items-center mb-2 px-2 py-1 border-2 border-gray-300 rounded-xl">
                            ...
                            <input onChange={onChangeHandler} className='text-base w-full pl-4 p-2 outline-none text-gray-500 bg-pureWhite ' placeholder='Enter your name' value={credentials.name} type="name" name="name" id="name" />
                        </div>
