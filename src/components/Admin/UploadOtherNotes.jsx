...
const UploadOtherNotes = () => {
    ...
    const handleFileUpload = async (e) => {
        try {
            setLoading(true);
            const fileInput = document.getElementById('notesOtherFile');
            ...
            const result = await storage.createFile(
                process.env.NEXT_PUBLIC_OTHER_NOTES_BUCKET_ID,
                ID.unique(),
                file
            );
    ...
    const handleInputSubmit = async () => {
        try {
            ...
            const result = await databases.createDocument(
                ...
                process.env.NEXT_PUBLIC_OTHER_COURSES_COLLECTION_ID,
                ID.unique(),
                {
    ...
    const CheckValidity = () => {
        if (notesDetails.name === '' || notesDetails.subject === '' || notesDetails.subjectCode === '' || notesDetails.unit === '' || notesDetails.courses === '') {
    ...
    const renderFileUpload = () => {
        if (!notesDetails.url) {
            return (
                <label className='preview flex flex-col items-center justify-center p-2 mb-8' htmlFor="notesOtherFile">
                    ...
                    <input onChange={handleFileUpload} className="hidden" type="file" name="notesOtherFile" id="notesOtherFile" />
                </label>
            );
        }
                ...
                <div className="container shadow-2xl shadow-black bg-[#D7D9DD] text-[#262626] font-jost mt-28 w-full rounded-2xl border-2 border-white">
                    <div className="content flex flex-col lg:flex-row">
                        <div className="heading lg:hidden block mt-6 px-7 font-bold">
                            <h1 className=" text-5xl lg:text-7xl font-jost">Upload other Notes</h1>
                        </div>
...
export default UploadOtherNotes;